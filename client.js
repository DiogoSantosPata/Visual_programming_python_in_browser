...
var client = {
    ...
    connect: function (port) {
        ...
        this.socket.onmessage = function (messageEvent) {
            ...
            else if(router === "stop_simulation")
            {
            }

            // If the response is from "count", do stuff
            else if (router === "countt"  ) 
            {
                current = $(".answer").html();
                if (current.length === 0) {
    ...
    countt: function () {
        var uuid = this.uuid();
        this.socket.send(JSON.stringify({method: "countt", id: uuid, params: {number: 0} }));  // adding something to params for it to work... fix it later
        this.queue[uuid] = "countt";
    },

    run: function (data) {
        var uuid = this.uuid();
        this.socket.send(JSON.stringify({method: "run", id: uuid, params: {diagram_structure: data} }));  // adding something to params for it to work... fix it later
        this.queue[uuid] = "run";
    },
    ...
    stop_simulation: function () {
        var uuid = this.uuid();
        this.socket.send(JSON.stringify({method: "stop_simulation", id: uuid, params: {} }));  // adding something to params for it to work... fix it later
        this.queue[uuid] = "stop_simulation";
    }
